...
#     return conn
# # Регистрация
# subprocess.run(["python", "telegramBot\\registration.py"])
#
...
#
#
# @bot.message_handler(func=lambda message: message.text == "/start_build")
...
#     ask_component(message, "processors")
#
# def ask_component(message, hui):
...
#     print(3)
#     min_price = distribution[hui]
#     print(4)
#     max_price = distribution[hui]
#     print(55)
#
#     conn = connect_db()
#     cursor = conn.cursor()
#     cursor.execute(f"""
#         SELECT id, name, price FROM {hui}
...
#
#     if not options:
#         bot.send_message(message.chat.id, f"Не найдено компонентов для {hui}. Пропускаем.")
#         next_component(message, hui)
#         return
#
#     keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
#         reply_markup=keyboard
#     )
#     bot.register_next_step_handler(message, lambda msg: save_component(msg, hui))
#
